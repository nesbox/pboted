...
const char *Logging::TimeAsString(std::time_t t) {
  if (t != m_LastTimestamp) {
    strftime(m_LastDateTime, sizeof(m_LastDateTime), m_TimeFormat.c_str(),
             localtime(&t));
    m_LastTimestamp = t;
  }
 ...
 */
void Logging::Process(std::shared_ptr<LogMsg> msg) {
  if (!msg)
    return;
...
void Logging::Append(std::shared_ptr<pbote::log::LogMsg> &msg) { m_Queue.Put(msg); }
...
void Logging::SendTo(std::shared_ptr<std::ostream> os) {
  m_HasColors = false;
...
#ifndef _WIN32
void Logging::SendTo(const char *name, int facility) {
  if (m_MinLevel == eLogNone)
    return;
  m_HasColors = false;
